...
class Kmeans(object):
    ...
    def calcDistance(self, a, b):  # 计算欧式距离作为判断聚类的依据
        result = numpy.sqrt(sum((a - b) ** 2))
        return result

    def shouldExit(self, iterations):  # 给出了循环停止的条件
...
if __name__=="__main__":
    ...
    path = 'F:/project/operatordata/pics/'#注意这个格式
    fp = open('F:/project/operatordata/file_color.txt','w')
    for filename in os.listdir(path):
        print (path+filename)
        
        color = k_image.run(Image.open(path+filename))#读入一个图像
